...
def comment():
    ...
    comment_add = "INSERT INTO comment_board(membername, comment) VALUES(%s, %s)"
    cursor.execute(comment_add, (comment,))
    mydb.commit()
    cursor.execute("SELECT content FROM message ORDER BY time DESC")
    all_comment = cursor.fetchall()
    
    return render_template("member.html", message = all_comment)
